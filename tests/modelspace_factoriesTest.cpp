...
TEST( ModelspaceFactories, PPSPFromSP ) {
    SingleParticleModelspace spms
        = read_sp_modelspace_from_file( "tests/data/ipm_modelspace.dat" );
    PPFromSP ppspms = build_ppsp_modelspace_from_sp( spms );
...
//        EXPECT_EQ( 20, ppspms[i].size() ); }
    ...
    EXPECT_EQ( 1, ppspms[0][6][0].size() );
    // These are the common particles, they have 2 0+/- states
    EXPECT_EQ( 2, ppspms[0][7][0].size() );
    EXPECT_EQ( 2, ppspms[0][8][0].size() );
    EXPECT_EQ( 2, ppspms[0][9][0].size() );
    // f7/2 proton particle
    EXPECT_EQ( 3, ppspms[1][6][0].size() );
    // These are the common particles
    EXPECT_EQ( 6, ppspms[1][7][0].size() );
    EXPECT_EQ( 5, ppspms[1][8][0].size() );
    EXPECT_EQ( 4, ppspms[1][9][0].size() );

    // f7/2 proton particle
    EXPECT_EQ( 5, ppspms[2][6][0].size() );
    // These are the common particles
    EXPECT_EQ( 7, ppspms[2][7][0].size() );
    EXPECT_EQ( 7, ppspms[2][8][0].size() );
    EXPECT_EQ( 4, ppspms[2][9][0].size() );
}

TEST( ModelspaceFactories, HHSPFromSP ) {
    SingleParticleModelspace spms
        = read_sp_modelspace_from_file( "tests/data/ipm_modelspace.dat" );
    PPFromSP hhspms = build_hhsp_modelspace_from_sp( spms );
...
//        EXPECT_EQ( 20, hhspms[i].size() ); }
    ...
    // Some J=0 states
    EXPECT_EQ(  6, hhspms[0][0][0].size() );
    EXPECT_EQ(  4, hhspms[0][1][0].size() );
    EXPECT_EQ(  6, hhspms[0][2][0].size() );
    EXPECT_EQ(  2, hhspms[0][3][0].size() );
    // Some J=1 states
    EXPECT_EQ( 10, hhspms[1][0][0].size() );
    EXPECT_EQ( 12, hhspms[1][1][0].size() );
    EXPECT_EQ( 10, hhspms[1][2][0].size() );
    EXPECT_EQ(  7, hhspms[1][3][0].size() );
}
